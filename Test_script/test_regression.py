...
gamalytic_steam_games = pd.read_csv('gamalytic_steam_games_reg.csv')
...
merged_data['steam_trading_cards'] = merged_data['steam_trading_cards'].fillna(False).astype(bool)
merged_data['workshop_support'] = merged_data['workshop_support'].fillna(False).astype(bool)
...
model = joblib.load('regression_catboost_model (1).pkl')
# catboost = joblib.load('regression_catboost_model.pkl')

from catboost import  CatBoostClassifier

x = [
'price_log', 'reviewScore_log', 'metacritic_log',  'steam_trading_cards', 'encoded_sorted_genres', 'has_demo', 'has_dlc',
      'workshop_support', 'achievements_total_log', 'is_free', 'encoded_supported_platforms', 'months_passed', 'metacritic_review_interaction'
]
X = merged_data[x]
print(X.isna().sum())
Y = merged_data['copiesSold']
